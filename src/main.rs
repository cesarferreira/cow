...
#[derive(Parser, Debug)]
#[command(name = "cow", version, about = "Create cheap independent directory clones")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn run(cli: Cli) -> ExitCode {
    match cli.command {
        Command::Clone { source, destination, strategy, require_cow, json, verbose } => {
            if require_cow && strategy == StrategyArg::Copy {
                eprintln!("error: --require-cow cannot be used with --strategy copy");
                return ExitCode::from(2);
            }
            let strategy = if require_cow { StrategyPreference::Cow } else { strategy.into() };
            match clone_dir(source, destination, CloneOptions { strategy }) {
                Ok(result) => {
                    if json { print_json(&CloneOutput::from(&result)); } else { print_clone(&result, verbose); }
                    ExitCode::SUCCESS
                }
        ...
        Command::Info { path, json } => match inspect(path) {
            Ok(info) => {
                if json { print_json(&InfoOutput::from(info)); } else { print_info(&info); }
                ExitCode::SUCCESS
            }
            Err(error) => print_error(&error, json),
        },
...
fn print_json(value: &impl Serialize) {
    println!("{}", serde_json::to_string(value).expect("serializable output"));
}

fn print_error(error: &CowError, json: bool) -> ExitCode {
    if json {
        let message = error.to_string();
        let envelope = ErrorEnvelope { error: ErrorOutput { code: error.code(), message: &message } };
        eprintln!("{}", serde_json::to_string(&envelope).expect("serializable error"));
    } else {
...
fn print_clone(result: &CloneResult, verbose: bool) {
    let verb = if result.strategy.is_cow() { "Cloned" } else { "Copied" };
    println!(
        "✓ {verb} {} → {} using {} ({})",
        result.source.display(), result.destination.display(), strategy_name(result.strategy),
...
fn print_info(info: &FilesystemInfo) {
    ...
    println!("Filesystem: {}", info.filesystem.as_deref().unwrap_or("unknown"));
    println!("Copy-on-write: {}", capability_name(info.cow_supported));
    println!("Preferred strategy: {}", strategy_name(info.preferred_strategy));
}
...
fn format_duration(milliseconds: u128) -> String {
    if milliseconds < 1_000 { format!("{milliseconds} ms") } else { format!("{:.1} s", milliseconds as f64 / 1_000.0) }
}
